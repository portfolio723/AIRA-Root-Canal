            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-12 gap-y-6 mb-16">
              {[
                ...
                <div key={i} className="flex items-center gap-4 group">
                  <div className="h-8 w-8 rounded-full bg-accent/20 flex items-center justify-center flex-shrink-0 group-hover:bg-accent transition-colors">
                    <CheckCircle2 className="h-5 w-5 text-primary" />
                  </div>
                  <span className="text-lg font-medium text-primary/80 group-hover:text-primary transition-colors">{symptom}</span>
                </div>
              ))}
            </div>

            <div className="text-center bg-primary/5 rounded-[40px] p-8 md:p-12 border border-primary/10">
              <p className="text-lg text-primary/80 mb-10 max-w-2xl mx-auto font-medium leading-relaxed">
                Don’t ignore the pain — early treatment saves your tooth and prevents complications.
              </p>
              <Button size="lg" className="bg-[#ff6b3d] hover:bg-[#e85a2d] text-white h-16 px-10 text-xl font-bold rounded-2xl shadow-xl transition-all hover:scale-105 active:scale-95 border-none">
